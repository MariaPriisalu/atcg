# The purpose of this script is to interact with the car environment
# This will be interacting directly with learning by cheating, carla default etc. - should keep it compatibile in terms of interfaces
...
## TODO: CIPRIAN : Leave here only what is needed
class RealTimeCarEnv(object):
	"""
		...
		if loc is not None:
			...
			if w is not None:
				...
				if current_lane_id not in [-1, 1]:
					...
					vehicle_hazard = True
					reward = -1.0 # To do: Put such constants with the sign in a settings file instead. Then it is easier to have an overview of what hyper paramters have been changed from one run to another.
			else:
				vehicle_hazard = True
				reward = -1.0
		else:
...
def playGame(env):
	...
	while frames > 0:
		...
		spectator.set_transform(carla.Transform(env.vehicle.get_location() + carla.Location(z=50), carla.Rotation(pitch=-90)))
		env.world.tick()
		obs, _, done, _ = env.step()
		frames -= 1

		if frames % 100 == 0:
			print(f"At frame {frames}")
...
def playDemo():
	...
	env = RealTimeCarEnv(args=variant)
	playGame(env)
