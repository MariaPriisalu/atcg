...
"""
CARLA No-Rendering Mode Visualizer
...
FrameToStartZOOMIn = 0

try:
    carlaEggPathPattern = os.path.join(CARLA_INSTALL_PATH, 'PythonAPI/carla/dist/carla-*%d.%d-%s.egg' % (
        sys.version_info.major,
        sys.version_info.minor,
        'win-amd64' if os.name == 'nt' else 'linux-x86_64'))

    carlaEggPath = glob.glob(carlaEggPathPattern)
    sys.path.append(carlaEggPath[0])
...
COLOR_SCARLET_RED_FULL = pygame.Color(255, 0, 0)
...
TRAINABLE_VEHICLE_COLOR = pygame.Color(255, 0, 0, 80)
TRAINABLE_PEDESTRIAN_COLOR = pygame.Color(0, 255, 255, 80)
...
HERO_DEFAULT_SCALE_ZOOMED_TO_OBSERVE = 0.5
...
class Util(object):
    ...
    @staticmethod
    def blits(destination_surface, source_surfaces, rect=None, area=None, blend_mode=0):
        """Function that renders the all the source surfaces in a destination source"""
        for surface in source_surfaces:
            destination_surface.blit(surface[0], surface[1], area=area, special_flags=blend_mode)
...
class HUD (object):
    ...
    def getInfoWidth(self):
        return self.info_width if self.show_info is True else 0

    def render(self, display):
        """If flag enabled, it renders all the information regarding the left panel of the visualizer"""
        if self.show_info:
            info_surface = pygame.Surface((self.info_width, self.dim[1]))
...
class MapImage(object):
    ...
    def __init__(self, carla_world, carla_map, pixels_per_meter, show_triggers, show_connections, show_spawn_points, useSegmentationMapForOnlineLevel, args=None):
        ...
        if args.onlineEnvSettings.useFixedWorldOffsets:
            min_x = args.onlineEnvSettings.fixedWorld_minX
            max_x = args.onlineEnvSettings.fixedWorld_maxX
            min_y = args.onlineEnvSettings.fixedWorld_minY
            max_y = args.onlineEnvSettings.fixedWorld_maxY
        else:
        ...
        width_in_pixels = int(math.ceil(self._pixels_per_meter * self.width))

        self.big_map_surface = pygame.Surface((width_in_pixels, width_in_pixels)).convert()

        dirname = os.path.join("cache", "no_rendering_mode")
        carla_map_namestr = pathlib.PurePath(carla_map.name).name

        if not useSegmentationMapForOnlineLevel:
            ...
            filename = carla_map_namestr + "_" + opendrive_hash + ".tga"
        else:
            filename = carla_map_namestr + "_" + "segmented" + ".png"
            imgContent = imageio.imread(os.path.join(dirname, filename))

            hash_func = hashlib.sha1()
            hash_func.update(imgContent)
            imgHash = str(hash_func.hexdigest())
            filename_original = filename
            filename = filename + "_" + imgHash + ".tga"
        ...
        if os.path.isfile(full_path):
            ...
            procedurallyBuiltSurface = self.big_map_surface
            if useSegmentationMapForOnlineLevel:
                procedurallyBuiltSurface.set_alpha(128)

            # Render map
            self.draw_road_map(
                procedurallyBuiltSurface,
                carla_world,
                carla_map,
                self.world_to_pixel,
            ...
            if useSegmentationMapForOnlineLevel:
                pygame.image.save(procedurallyBuiltSurface, os.path.join(dirname, "temp_onlyProcedural.png"))
                self.big_map_surface = pygame.image.load(os.path.join(dirname, filename_original))
                self.big_map_surface.blit(procedurallyBuiltSurface, (0,0))
            ...
            #list_filenames = glob.glob(os.path.join(dirname, carla_map_namestr) + "*")
    ...
    def world_to_pixel_noscale(self, location, offset=(0, 0)):
        """Converts the world coordinates to pixel coordinates"""
    ...
    def scale_map(self, scale, smooth=True):
        ...
        if scale != self.scale:
            ...
            if smooth:
                ...
                self.surface = pygame.transform.scale(self.big_map_surface, (width, width))


class World(object):
    """Class that contains all the information of a carla world that is running on the server side"""

    def __init__(self, name, args, timeout, hud):
        self.client = None
        self.name = name
        self.args = args
        self._hud = hud
        ...
        self.debug_savedScaledMap = False # False

        self.mouseControlEnabled = args.onlineEnvSettings.isDataGatherAndStoreEnabled
        if self.mouseControlEnabled:
            global MAP_DEFAULT_SCALE
            MAP_DEFAULT_SCALE = 0.1

        if args.onlineEnvSettings.useFixedWorldOffsets:
            global PIXELS_PER_METER
            PIXELS_PER_METER = args.onlineEnvSettings.fixedPixelsPerMeter
        ...
        self.sceneCenter = [0, 0] # Where is the focus center now

        # These two will be set on not None failure attempts to set scene center because
        self.setSceneCenter_pending_bboxWindow = None

        self.fixedWindowSize_X = 500 # Scaled dimensions for what to observe from the scene
        self.fixedWindowSize_Y = 500
        self.thresholdForScaleChangeAtRuntime = 0.2 # rescale the bk image if larger than this

        # Where should the drawing of the scene begin in the current resolution setup
        self.drawSceneOnHUD_Offset = (self._hud.getInfoWidth(), 0)

        #  The dimension of remainings for scene drawing in the current resolution setup
        self.drawSceneOnHud_Dims_X = self._hud.dim[0] - self.drawSceneOnHUD_Offset[0]
        self.drawSceneOnHud_Dims_Y = self._hud.dim[1] - self.drawSceneOnHUD_Offset[1]

        self.drawSceneOnHud_Dims_half_X = self.drawSceneOnHud_Dims_X*0.5
        self.drawSceneOnHud_Dims_half_Y = self.drawSceneOnHud_Dims_Y*0.5

        self.scale_offset = [0, 0]
        self.currentScaleTEMP = 0 # GET RID !
        ...
        self.initPosDone = False
        self.frameCounter = 0

        self.actorsContext = None # Management structure copied from above for handling actors locations, ids, trainable info etc

    def _get_data_from_carla(self):
        """Retrieves the data from the server side"""
        try:
            self.client = carla.Client(self.args.onlineEnvSettings.host, self.args.onlineEnvSettings.port)
            self.client.set_timeout(self.timeout)

            if self.args.onlineEnvSettings.map is None:
                world = self.client.get_world()
            else:
                world = self.client.load_world(self.args.onlineEnvSettings.map)
    ...
    # If the connection to carla was already made, give it here the spawned world and map loaded
    def start(self, input_control, alreadySpawnedWorld = None, alreadySpawnedMapName = None, actorsContext = None):
        """Build the map image, stores the needed modules and prepares rendering in Hero Mode"""
        self.actorsContext = actorsContext
        if alreadySpawnedWorld is not None:
            self.world, self.town_map = alreadySpawnedWorld, alreadySpawnedMapName
        else:
            self.world, self.town_map = self._get_data_from_carla()
            settings = self.world.get_settings()
            settings.no_rendering_mode = self.args.onlineEnvSettings.no_server_rendering
        ...
        self.map_image = MapImage(
            ...
            show_triggers=self.args.onlineEnvSettings.show_triggers,
            show_connections=self.args.onlineEnvSettings.show_connections,
            show_spawn_points=self.args.onlineEnvSettings.show_spawn_points,
            useSegmentationMapForOnlineLevel=self.args.onlineEnvSettings.useSegmentationMapForOnlineLevel,
            args=self.args)
        ...
        if self.setSceneCenter_pending_bboxWindow is not None:
            self.setSceneWindow(self.setSceneCenter_pending_bboxWindow)

            self.setSceneCenter_pending_bboxWindow = None

        # Render Actors
        self.actors_surface = pygame.Surface((self.map_image.surface.get_width(), self.map_image.surface.get_height())).convert()
        ...
        if self.args.onlineEnvSettings.use_hero_actor_forDataGathering:
            ...
            self._input.wheel_offset = HERO_DEFAULT_SCALE #HERO_DEFAULT_SCALE_ZOOMED_TO_OBSERVE
    ...
    def _spawn_hero(self):
        ...
        blueprint = random.choice(self.world.get_blueprint_library().filter(self.args.onlineEnvSettings.filter))
    ...
    def tick(self, clock, worldSnapshot=None):
        if worldSnapshot is None:
            snapshot = self.world.get_snapshot()
        else:
            snapshot = worldSnapshot

        World.on_world_tick(self, snapshot)
    ...
    def update_hud_info(self, clock):
        ...
        info_text = [
            ...
            'Simulation Frame: % 10d' % self.simulation_frame,
    ...
    # The center location and objects to visualize in the world
    def setSceneWindow(self, bbox : BBox2D):
        if self.map_image is None:
            self.setSceneCenter_pending_bboxWindow = bbox
            return

        center = bbox.getCenter()
        width, depth = bbox.getDims()
        self.sceneCenter = self.map_image.world_to_pixel(carla.Location(center[0], center[1]))

        # How many meters do we want to see from the original image of the scene to the screen space dedicated
        targetSceneX_pixels = width * PIXELS_PER_METER
        targetSceneY_pixels = depth * PIXELS_PER_METER

        howManyPixelsToSee = max(targetSceneX_pixels, targetSceneY_pixels)
        howManyPixelsAvailable = min(self.drawSceneOnHud_Dims_X, self.drawSceneOnHud_Dims_Y)
        assert howManyPixelsAvailable > 0 and howManyPixelsToSee > 0

        targetedScaleRatio = howManyPixelsAvailable / howManyPixelsToSee #min(howManyPixelsAvailable / howManyPixelsToSee, 1.0)

        # TODO check thresholds
        if abs(targetedScaleRatio - self.currentScaleTEMP) > self.thresholdForScaleChangeAtRuntime:
            self.currentScaleTEMP = targetedScaleRatio
            self._compute_scale(targetedScaleRatio)
    ...
    def on_world_tick(self, snapshot):
        ...
        self.simulation_frame = snapshot.frame
    ...
    def _render_traffic_lights(self, surface, list_tl, world_to_pixel):
        ...
        for tl in list_tl:
            ...
            if self.args.onlineEnvSettings.show_triggers:
    ...
    def _render_walkers(self, surface, list_w, world_to_pixel):
        ...
        for w in list_w:
            color = COLOR_SCARLET_RED_FULL # COLOR_PLUM_0
            ...
            # Show a circle around the trainable agents
            walkerCenter = world_to_pixel(w[1].location)
            if w[0].id in self.actorsContext.trainableWalkerIds:
                pygame.draw.circle(surface, TRAINABLE_PEDESTRIAN_COLOR, walkerCenter, 10, width=1)
    ...
    def _render_vehicles(self, surface, list_v, world_to_pixel):
        ...
        for v in list_v:
            ...
            # Show a circle around the trainable agents
            vehicleCenter = world_to_pixel(v[1].location)
            if v[0].id in self.actorsContext.trainableVehiclesIds:
                pygame.draw.circle(surface, TRAINABLE_VEHICLE_COLOR, vehicleCenter, 10, width=1)

    def render_actors(self, surface, vehicles, traffic_lights, speed_limits, walkers):
        """Renders all the actors"""
        # Static actors
        if self.args.onlineEnvSettings.renderTrafficLights == 1:
            self._render_traffic_lights(surface, [tl[0] for tl in traffic_lights], self.map_image.world_to_pixel)
        if self.args.onlineEnvSettings.renderSpeedLimits == 1:
    ...
    def clip_surfaces(self, clipping_rect, resultsurface_clipping_rect=None):
        ...
        if resultsurface_clipping_rect is not None:
            self.result_surface.set_clip(resultsurface_clipping_rect)
    ...
    def _compute_scale(self, scale_factor):
        ...
        # Scale performed
        self.map_image.scale_map(scale_factor, smooth=True if self.mouseControlEnabled else False)

    def render(self, display, worldSnapshot=None, isInitializationFrame=False):
        ...
        # Zoom in and out
        if self.mouseControlEnabled:
            scale_factor = self._input.wheel_offset if self.mouseControlEnabled is True else HERO_DEFAULT_SCALE_ZOOMED_TO_OBSERVE
            self.scaled_size = self.map_image.big_map_surface.get_width() * scale_factor #int(self.map_image.width * scale_factor)
            if self.scaled_size != self.prev_scaled_size or isInitializationFrame:
        ...
        if self.hero_actor is not None:
            ...
            """
            The algorithms and rendering idea:
            A. From external, we'll get a  call to setSceneWindow(window bbox) containing the bounding box of the scene to draw from the real world.
                - We do internally scaling of the map, computing the sceneCenter, etc,
                            
            B. Rendering process
             - we first use the sceneCenter to know where is the center of the scene to draw according to current SCALED MAP image
             - we select from the SCALED MAP, using drawMapDims a subspace of  self.drawSceneOnHud_Dims_X, self.drawSceneOnHud_Dims_Y which is the output drawing on the resulted window
             - this is our clipping_rect.
             - We then draw this rect on the self.result_surface, starting from 0,0 and up to self.drawSceneOnHud_Dims_X, self.drawSceneOnHud_Dims_Y.
             - The result_surface will then be drawn using the offset in the windows targeted by drawSceneOnHUD_Offset (which represent where the map HUD is in the output window).
            """
            ...
            if self.mouseControlEnabled:
                ...
                clipping_rect = pygame.Rect(-translation_offset[0] - center_offset[0], -translation_offset[1]- center_offset[1], self._hud.dim[0], self._hud.dim[1])
            else:
                drawMapDims =  (self.sceneCenter[0] - self.drawSceneOnHud_Dims_half_X,
                                self.sceneCenter[1] - self.drawSceneOnHud_Dims_half_Y,
                                self.drawSceneOnHud_Dims_X,
                                self.drawSceneOnHud_Dims_Y)

                clipping_rect = pygame.Rect(drawMapDims[0], drawMapDims[1], self.drawSceneOnHud_Dims_X, self.drawSceneOnHud_Dims_Y)

            # TODO: fix the semaphore and ids showing surface
            # TODO: fix min/max scaling dimensions (i.e. not allowing less than X meters).
            # Apply clipping rect to surfaces and apply them
            self.clip_surfaces(clipping_rect, resultsurface_clipping_rect=pygame.Rect(0, 0, self.drawSceneOnHud_Dims_X, self.drawSceneOnHud_Dims_Y))
            Util.blits(self.result_surface, surfaces, rect=None, area=clipping_rect)
            #self.result_surface.blit(surfaces[0][0], dest=surfaces[0][1], area=clipping_rect)#, special_flags=blend_mode)

            # Just to check frame related issue for debugging
            if self.debug_savedScaledMap is False:
                self.debug_savedScaledMap = True
                pygame.image.save(surfaces[0][0], os.path.join("temp_surface0.png"))
                pygame.image.save(surfaces[1][0], os.path.join("temp_surface1.png"))
                pygame.image.save(surfaces[2][0], os.path.join("temp_surface2.png"))
                pygame.image.save(self.result_surface, os.path.join("temp_resultsurface.png"))

            # Blit the result to the display
            if self.mouseControlEnabled:
                display.blit(self.result_surface, (translation_offset[0] + center_offset[0], translation_offset[1] + center_offset[1]))
            else:
                display.blit(self.result_surface, dest=self.drawSceneOnHUD_Offset)#, area=drawMapDims)


            # Simulate a zoom at the observer position in the next frame
            if False:
                if self.frameCounter > FrameToStartZOOMIn and (self.initPosDone is False or isInitializationFrame == True):
                    observerPosRatio_X, observerPosRatio_Y = ((self.sceneCenter[0] - self.map_image._world_offset[0]) / self.map_image.width,
                                                              (self.sceneCenter[1] - self.map_image._world_offset[1]) / self.map_image.width)
                    #observerDisplay_X, observerDisplay_Y = observerPosRatio_X * self.surface_size, observerPosRatio_Y * self.surface_size

                    self.initPosDone = True
                    self._input.pending_customZoomAndPos = True
                    self._input.pending_ZoomFactor = 0.9
                    self._input.pending_initialMouseX = center_offset[0] + display.get_width() * observerPosRatio_X
                    self._input.pending_initialMouseY = display.get_height() * observerPosRatio_Y
...
class InputControl(object):
    ...
    def __init__(self, name, args):
        ...
        # Zoom at a certain target in a hacked way
        self.pending_customZoomAndPos = False
        self.pending_ZoomFactor = None
        self.pending_initialMouseX = None
        self.pending_initialMouseY = None
    ...
    def _parse_events(self):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                    ...
                    if self._world.hero_actor is None:
                        ...
                        self.wheel_offset = HERO_DEFAULT_SCALE_ZOOMED_TO_OBSERVE #MAP_DEFAULT_SCALE
            ...
            elif self.mouseControlEnabled and (self.pending_customZoomAndPos == True or event.type == pygame.MOUSEBUTTONDOWN):
                # Handle mouse wheel for zooming in and out
                if self.pending_customZoomAndPos == True or event.button == 4:
                    self.pending_customZoomAndPos = False
                ...
                print(f"New Wheel offset {self.wheel_offset} at offset mouse {self.mouse_pos}")

        if self.mouseControlEnabled and self.pending_customZoomAndPos is True:
            self.pending_customZoomAndPos = False
            self.wheel_offset = self.pending_ZoomFactor
            self.mouse_pos = (self.pending_initialMouseX, self.pending_initialMouseY)
...
def game_loop(args):
    ...
    try:
        ...
        world = World(TITLE_WORLD, args, timeout=2.0, hud=hud)
