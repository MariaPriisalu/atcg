...
sys.path.append("RLAgent/commonUtils/RealTimeEnv")

print("ADD TO path "+str(os.path.dirname( os.path.abspath(__file__)))+" " +str(os.path.join(os.path.dirname( os.path.abspath(__file__)), 'RealTimeEnv'))+" --------------------------------")
sys.path.append(os.path.dirname( os.path.abspath(__file__)))
sys.path.append(os.path.join(os.path.dirname( os.path.abspath(__file__)), 'RealTimeEnv'))
...
cityscapes_labels = ['unlabeled', # 0
                     ...
                     'train', # 31
                     'motorcycle', # 32
                     'bicycle'] # 33 # note last semantic channel is not used: licence plate, thus not in list.
...
# Frame in people and cars trajectory that describes some detailed spawning information
...
# Gets a translation from a filepath - not needed anymore, we get the full transform 
def get_translation(filepath, realtime_data=False):
    translation = [0.0, 0.0, 0.0] # Default if no translation exists
    if filepath is not None and os.path.exists(filepath) and not realtime_data:
...
def get_camera_matrix(filepath):
    ...
    cameras_path_alternative = os.path.join(filepath, 'cameras_replay.p')
    ...
    elif os.path.exists(cameras_path_alternative):
        # THis version contain directly the transform from world to observer position - in world space still
        cameras = pickle.load(open(cameras_path_alternative, "rb"), encoding="latin1", fix_imports=True)
        frames = [x for x in range(1000)]
        translation  = cameras[0:3, 3]
        rotation = cameras[0:3, 0:3]
        return rotation, translation, frames
...
def get_cars_list(list, cars_dict, frames, WorldToCameraRotation, WorldToCameraTranslation,
                  heightOffset, scale, K, filepath, people_flag=False, find_poses=False, datasetOptions=None, outTrainableIndices=None):
    image_size = [800, 600]
    list_2D = []
    poses = []
    ...
    """
    if len(cars_dict) == 0:
        print("Frames in car None")
    else:
        print(" Frames in car "+str(max(cars_dict.keys()))+" min frame "+str(min(cars_dict.keys())))
    """

    if FRAME_INDEX_FOR_SPAWN_DETAILS in cars_dict:
        del cars_dict[FRAME_INDEX_FOR_SPAWN_DETAILS]

    for i, frame in enumerate(sorted(cars_dict.keys())):
        assert frame != FRAME_INDEX_FOR_SPAWN_DETAILS

        # If there is some dataset options and there is a number of items limit..
        if datasetOptions.isCustom is True and i >= datasetOptions.LIMIT_FRAME_NUMBER:
            break

        frame_cars = cars_dict[frame]
        list_2D.append([])
        if outTrainableIndices is not None:
            outTrainableIndices.append([])
        poses.append([])
        for car_id in frame_cars:
            ...
            if False:  # 'bounding_box_coord' in frame_cars[car_id].keys() and not people_flag:
                ...
                dataAlreadyInGlobalSpace = datasetOptions.isCustom is True and datasetOptions.dataAlreadyInWorldSpace is True
            ...
            if outTrainableIndices is not None:
                isEntityIdTrainable = None
                if people_flag:
                    isEntityIdTrainable = car_id in datasetOptions.trainableWalkerIds
                else:
                    isEntityIdTrainable = car_id in datasetOptions.trainableVehiclesIds
                if isEntityIdTrainable:
                    if outTrainableIndices[i] is None:
                        outTrainableIndices[i] = []
                    outTrainableIndices[i].append(len(list[i]) - 1)
...
def objects_in_range(objects, pos_x, pos_y, depth, width, to_int=False, constantCarsandPedestrians=False,
                     trainableIndices=None):
    ...
    # print(" Check that people in range? width: "+str(width)+" depth "+str(depth)+" pos x "+str(pos_x)+" pos y "+str(pos_y))
    for indx, objs_frame in enumerate(objects):
        objs_sample.append([])
        if objs_frame is None:
            continue

        for objIndex, obj in enumerate(objs_frame):
            ...
            # Either is a trainable object index or is in the range
            isobject_in_x_range=((pos_x <= x_min and x_min <pos_x + depth) or (pos_x <= x_max and x_max < pos_x + depth))
            isobject_in_y_range = ((pos_y <= y_min and  y_min <pos_y + width ) or ( pos_y <= y_max and y_max < pos_y + width))
            isObjectInRange = (isobject_in_x_range and isobject_in_y_range)
            idinTrainableObjects=trainableIndices and (objIndex in trainableIndices[indx])

            if isObjectInRange :#or idinTrainableObjects:
                if len(obj) == 6 :
                ...
                # print("Is object in range " + str(isObjectInRange) +  " obj " + str(len(objs_sample[indx])))
        all_objs.extend(objs_sample[indx])

    if constantCarsandPedestrians:
        # This code is very obsolete ! please let's fix !<----- This is for cityscapes! When the cars and pedestrians must be still due to noise in their motion estimation.
        for indx, objs_frame in enumerate(objects):
...
def objects_in_range_map(objs_dict, pos_x, pos_y, depth, width, to_int=False, init_frames= {},
    ...
    # print ("Trainable ids "+str(trainableIds))
    # print (objs_dict)
    ...
    for key, objs_frame in list(objs_dict.items()):
        ...
        for i, obj in enumerate(objs_frame):
            ...
            # isObjInRange =  ((pos_x <= x_min and x_min <pos_x + depth) or (pos_x <= x_max and x_max < pos_x + depth)) \
            #         and ((pos_y <= y_min and  y_min <pos_y + width ) or ( pos_y <= y_max and y_max < pos_y + width)) or (key in trainableIds)
            isobject_in_x_range = (
                        (pos_x <= x_min and x_min < pos_x + depth) or (pos_x <= x_max and x_max < pos_x + depth))
            isobject_in_y_range = (
                        (pos_y <= y_min and y_min < pos_y + width) or (pos_y <= y_max and y_max < pos_y + width))
            isObjectInRange = (isobject_in_x_range and isobject_in_y_range)
            idinTrainableObjects =  trainableIds and (key in trainableIds)
            ...
            if isObjectInRange or idinTrainableObjects:
                ...
                if len(obj) == 6 :
                    ...
                    objs_in_range_dictionary[key].append(
                        obj[[2, 1, 0], :] - np.tile(np.array([0,pos_y, pos_x]).reshape((3, 1)), (1, obj.shape[1])))
                # print("Is object in range " + str(isObjectInRange) + " trainable: " + str(idinTrainableObjects) + " len init keys " + str(len(objs_in_range_dictionary)))
...
def get_cars_map(cars_dict, frames, R, middle, height, scale, K, filepath, people_flag=False, find_poses=False,
    ...
    for i, frame in enumerate(frames):
        ...
        for car_id in frame_cars:
            ...
            if datasetOptions.isCustom is False or datasetOptions.dataAlreadyInWorldSpace is False:
...
# There are two ways to give people and cars data to this:
# A. by filling a file path where data with the two files with people and cars motion existis
# B. by feeding data already processed from an external data (the params with suffix _external)
# Data should be in the same format, and Only one of A or B is considered at one time. assert will raise otherwise
# Format of the data :  {frame id : { frame_data }} where frame_data is { entity_id : { bboxminmax : 3x2 floats, min and max per each axis and row in order X,Y,Z}, velocity : 4x1}}
def get_people_and_cars(WorldToCameraRotation, cars, filepath, frames, WorldToCameraTranslation, heightOffset,
                        ...
                        carsData_external=None,
                        peopleData_external=None):
    ...
    K = None
    if (datasetOptions.isCustom is False or datasetOptions.dataAlreadyInWorldSpace is False) and  datasetOptions.realtime_data ==False:
        K = get_camera_intrinsic_matrix(filepath)
    load_imports=True
    if datasetOptions.realtime_data and datasetOptions.realtime_dataOnline==False:
        load_imports=False
    print("Load imports "+str(load_imports)+" ----------------------------------------")

    # Read the cars and people dictionary in format: {frame_id : { entity_id : {...dict with data... } } }
    #-------------------------------------------------------------------------------------------------
    if filepath is not None:
        assert carsData_external is None and peopleData_external is None, "Only one data source should be given"
        carsdata_filepath = os.path.join(filepath, FILENAME_CARS_TRAJECTORIES)
        print (carsdata_filepath)
        with open(carsdata_filepath, 'rb') as handle:
            cars_dict_fromData = pickle.load(handle, encoding="latin1", fix_imports=load_imports)
        if simplifyDataSet == True and len(cars_dict_fromData) > 1:
            cars_dict_new = {}
            for x in range(len(frames)):
                cars_dict_new[x] = cars_dict_fromData[x]
            cars_dict_fromData = cars_dict_new
            with open(carsdata_filepath, 'wb') as handle:
                pickle.dump(cars_dict_fromData, handle, protocol=2)  # Protocol 2 ensures compatibility between python 2 and 3

        peopledata_filepath = os.path.join(filepath, FILENAME_PEOPLE_TRAJECTORIES)
        print(peopledata_filepath)
        with open(peopledata_filepath, 'rb') as handle:
            people_dict_fromData = pickle.load(handle, encoding="latin1", fix_imports=load_imports)

        if simplifyDataSet == True and len(people_dict_fromData) > 1:
            people_dict_new = {}
            for x in range(len(frames)):
                if x in people_dict_fromData:
                    people_dict_new[x] = people_dict_fromData[x]
                else:
                    people_dict_new[x] = {}

            people_dict_fromData = people_dict_new
            with open(peopledata_filepath, 'wb') as handle:
                pickle.dump(people_dict_fromData, handle, protocol=2)  # Protocol 2 ensures compatibility between python 2 and 3
    else:
        assert filepath is None, "Only one data source should be given"
        cars_dict_fromData = carsData_external
        people_dict_fromData = peopleData_external
    # print("Load from file "+peopledata_filepath+" len "+str(len(people_dict_fromData)))
    #-------------------------------------------------------------------------------------------------

    # The code below convert from motion data (given above) of both cars and pedestrians in this order:
    # (A) to  reconstruction space (optional step) - scaling, removing offsets, etc - in get_cars_list
    # (B) to voxel space coordinates (inversed axis) - in get_cars_map
    # (C) keep only in range entities following an origin position and depth/width - get_cars_map
    outCarIndices = datasetOptions.outCarIndices
    outPeopleIndices = datasetOptions.outPedestrianIndices
    cars, cars_2D = get_cars_list(cars, cars_dict_fromData, frames, WorldToCameraRotation, WorldToCameraTranslation,
                                  heightOffset, scale_x, K, filepath, find_poses=find_poses,
                                  datasetOptions=datasetOptions, outTrainableIndices=outCarIndices)
    people, people_2D = get_cars_list(people, people_dict_fromData, frames, WorldToCameraRotation, WorldToCameraTranslation,
                                      heightOffset, scale_x, K, filepath, people_flag=True, find_poses=find_poses,
                                      datasetOptions=datasetOptions, outTrainableIndices=outPeopleIndices)
    #print (" People "+str(people))
    numValidFrames = min(len(cars_dict_fromData), len(people_dict_fromData))
    ...
    # cars_dict_fromData = {car_id -> list of bbox ordered by frame | bboxes are ooab in voxels, [xmin, xmax+1, ymin, ymax+1, zmin, zmax+1] world coordinate}
    ...
    ped_dict, poses, valid_ids, init_frames = get_cars_map(people_dict_fromData, frames, WorldToCameraRotation,
                                                           WorldToCameraTranslation, heightOffset, scale_x, K, filepath,
                                                           people_flag=True, datasetOptions=datasetOptions)
    # print("After get cars map  " + str(len(ped_dict)))

    car_dict, _, _, init_frames_cars = get_cars_map(cars_dict_fromData, frames, WorldToCameraRotation, WorldToCameraTranslation,
                                                    heightOffset, scale_x, K, filepath, people_flag=False,
                                                    datasetOptions=datasetOptions)

    # Now filter the entities in range of the episode center
    # print (" Before adding people "+str(len(people[0])) )
    cars_sample = objects_in_range(cars, datasetOptions.pos_x, datasetOptions.pos_y, datasetOptions.depth, datasetOptions.width,
                                   constantCarsandPedestrians=datasetOptions.isOldDatasetRangeFormat, trainableIndices=outCarIndices)
    people_sample = objects_in_range(people, datasetOptions.pos_x, datasetOptions.pos_y, datasetOptions.depth, datasetOptions.width,
                                     constantCarsandPedestrians=datasetOptions.isOldDatasetRangeFormat, trainableIndices=outPeopleIndices)
    # print(" After adding people " + str(len(people_sample[0])))
    # people_dict_sample
    people_dict_sample = {}
    if len(ped_dict) > 0:
        # print("Run objects in range------------------------------------------------------------------"+str(len(ped_dict)))
        people_dict_sample, init_frames = objects_in_range_map(ped_dict, datasetOptions.pos_x, datasetOptions.pos_y,
                                                               datasetOptions.depth, datasetOptions.width, init_frames=init_frames,
                                                               trainableIds=datasetOptions.trainableWalkerIds)
    # print(" After objects_in_range_map   " + str(len(people_dict_sample)))
    # for key in people_dict_sample.keys():
    #     print(key)
    cars_dict_sample = {}
    if len(car_dict) > 0:
        cars_dict_sample, init_frames_cars = objects_in_range_map(car_dict, datasetOptions.pos_x, datasetOptions.pos_y,
                                                                  datasetOptions.depth, datasetOptions.width, init_frames=init_frames_cars,
                                                                  trainableIds=datasetOptions.trainableVehiclesIds)

    # By default overwrite the cars, people data by the sampled in range of the episode map considered
    cars = cars_sample
    people = people_sample
    ped_dict = people_dict_sample
    car_dict = cars_dict_sample
    # print(" Done with reconstruction   " + str(len(ped_dict)))
...
def combine_frames(WorldToCameraRotation, filepath, WorldToCameraTranslation,
    ...
    print(f"NumPoints expected {numPoints}")
...
# Given a world tensor in voxelized space this function does some post processing such as dilating some areas or filtering
def processWorldReconstructionTensor(tensor, datasetOptions):
    if datasetOptions.filter2D == True:
            ...
            for label in labels:
                ...
                for i in range(len(positions[0])):
                    if tensor[positions[0][i], positions[1][i], positions[2][i], CHANNELS.semantic] == 0:
                        tensor[positions[0][i], positions[1][i], positions[2][
                            i], CHANNELS.semantic] = label / NUM_SEM_CLASSES
        else:
            for channel in range(4):
                tensor[:, :, :, channel] = scipy.ndimage.filters.median_filter(tensor[:, :, :, channel], size=(
                1, 4, 4))  # (1,3,3), (2,3,3) or (1,4,4) which is more realistic?
...
def reconstruct3D_ply(filepath, local_scale_x=5, recalculate=False, find_poses=False, read_3D=True,
    ...
    assert datasetOptions is not None, "This is not allowed now to be None. Use the default options with configured isCustom bool API"

    FRAMEINDEX_MIN, FRAMEINDEX_MAX = None, None
    frames = datasetOptions.framesIndicesCaptured if datasetOptions.realtime_data != True else []
    frameSkip = datasetOptions.frameSkip
    WorldToCameraRotation = datasetOptions.WorldToCameraRotation
    numInitialFramesToIgnore = datasetOptions.numInitialFramesToIgnore
    WorldToCameraTranslation = datasetOptions.WorldToCameraTranslation

    numFramesExpected = 0
    if datasetOptions.realtime_data != True:
        FRAMEINDEX_MIN = int(min(frames))
        FRAMEINDEX_MAX = int(max(frames))
        print (" min frames "+str(FRAMEINDEX_MIN)+" max "+str(FRAMEINDEX_MAX))

        numFramesExpected = (FRAMEINDEX_MAX - min(0, FRAMEINDEX_MIN) + 1)

    people = [None]*numFramesExpected
    cars = [None]*numFramesExpected
    ...
    # Build the final tensor constructed as well
    pos_x = datasetOptions.pos_x
    pos_y = datasetOptions.pos_y
    begin_z = datasetOptions.begin_z
    envheight = datasetOptions.height
    envwidth = datasetOptions.width
    envdepth = datasetOptions.depth
    final_tensor = np.zeros((datasetOptions.height, datasetOptions.width, datasetOptions.depth, 6))

    def addPointToRecTensor(x, y, z, r, g, b, l, max_points=[0,0,0], min_points=[0,0,0]):
        z, y, x = int(z - begin_z), int(y - pos_y), int(x - pos_x)

        max_points[0] = max(max_points[0], z)
        max_points[1] = max(max_points[1], y)
        max_points[2] = max(max_points[2], z)
        min_points[0] = min(min_points[0], z)
        min_points[1] = min(min_points[1], y)
        min_points[2] = min(min_points[2], z)

        if z < 0 or y < 0 or x < 0 or z >= envheight or y >= envwidth or x >= envdepth:
            return

        final_tensor[z, y, x, CHANNELS.rgb[0]:CHANNELS.rgb[1]] = [r * (1/255.0), g * (1/255.0), b * (1/255.0)]

        if not isLabelIgnoredInReconstruction(l, isCarlaLabel=False):  # The input data is in cityscapes !
            final_tensor[z, y, x, CHANNELS.semantic] = l * (1 / NUM_SEM_CLASSES)
    ...
    if read_3D:
        ...
        if os.path.isfile(reconstruction_path) and os.path.isfile(centeringFilePath) and not recalculate:
            ...
            for point in pointcloud_3D:
                labelForPoint = getLabelFromPoint(point)
                Rvalue, Gvalue, Bvalue = point[3], point[4], point[5]
                Xvalue, Yvalue, Zvalue = int(point[0]), int(point[1]), int(point[2])
                final_reconstruction[(Xvalue, Yvalue, Zvalue)] = (Rvalue, Gvalue, Bvalue, labelForPoint) #(*getRgbFromPoint(point), getLabelFromPoint(point))
                addPointToRecTensor(x=Xvalue, y=Yvalue, z=Zvalue, r=Rvalue, g=Gvalue, b=Bvalue, l=labelForPoint,max_points=max_points, min_points=min_points)
            ...
            isFrame_0_needed = (datasetOptions.isCustom is True and datasetOptions.dataAlreadyInWorldSpace == True)
            ...
            for point in reconstruction_rgb:
                ...
                Xvalue, Yvalue, Zvalue = int(point[0]), int(point[1]), offsetZAxisKey(point[2], height_asint)
                voxelized_point_pos = (Xvalue, Yvalue, Zvalue)
                ...
                addPointToRecTensor(x=Xvalue, y=Yvalue, z=Zvalue, r=rgb[0], g=rgb[1], b=rgb[2], l=mapped_label)
        ...
        for point in pointcloud_3D:
            Rvalue, Gvalue, Bvalue = point[3], point[4], point[5]
            labelForPoint = point[6]
            Xvalue, Yvalue, Zvalue = int(point[0]), int(point[1]), int(point[2])
            final_reconstruction[(Xvalue, Yvalue, Zvalue)] = (Rvalue, Gvalue, Bvalue, labelForPoint)  # (*getRgbFromPoint(point), getLabelFromPoint(point))
            addPointToRecTensor(x=Xvalue, y=Yvalue, z=Zvalue, r=Rvalue, g=Gvalue, b=Bvalue, l=labelForPoint)

    # Adding this as augmented data to be used later if needed
    centering['worldToCameraRotation'] = WorldToCameraRotation
    datasetOptions.centering = centering

    # Getting the people and cars motion data from recorded dataset. If online env, do nothing
    # See the comments inside the called function, before return to understand meaning of all these
    motionDataBasePath = filepath

    if datasetOptions.realtime_dataOnline:
        carsData_external = {} # Empty dict since environment is spawned later
        peopleData_external = {}
        motionDataBasePath = None
    else:
        carsData_external = None # This will force reading from off line stored data in the function below
        peopleData_external = None
    cars, people, ped_dict, cars_2D, people_2D, poses, valid_ids, car_dict, init_frames, init_frames_cars = get_people_and_cars(
            ...
            datasetOptions=datasetOptions,
            carsData_external=carsData_external,
            peopleData_external=peopleData_external)
    # print("Reconstruction done len of ped dict "+str(len(ped_dict)))
    if find_poses:
        save_2D_poses(filepath, people_2D, frames)

    # Some final post processing...
    processWorldReconstructionTensor(tensor=final_tensor, datasetOptions=datasetOptions)

    return final_reconstruction, final_tensor, people, cars, local_scale_x, ped_dict, cars_2D, people_2D, valid_ids, car_dict, init_frames, init_frames_cars, centering


class DatasetOptions:
    def __init__(self):
        # This means that this is the default object, untouched
        self.isCustom = False

        self.dataAlreadyInWorldSpace = True  # Is data already converted to world space ?
        self.framesIndicesCaptured = None
        self.LIMIT_FRAME_NUMBER = None
        ...
        self.numInitialFramesToIgnore = None
        ...
        self.realtime_dataOnline = False
        self.isOldDatasetRangeFormat = False # Odd thing but required for some previous functionality...
        self.WorldToCameraRotation = None
        self.WorldToCameraTranslation = None
        self.centering = None # Will be set when reconstruciton is done (may contain some redundant info but that's fine...)

        # The current set of online ids of trainable pedestrians and cars666
        self.trainableWalkerIds = set()
        self.trainableVehiclesIds = set()
        self.outCarIndices=[]
        self.outPedestrianIndices = []
        # World specs, center and dimensions
        self.pos_x = None
        self.pos_y = None
        self.width = None
        self.height = None
        ...
        self.debugFullSceneRaycastRun = False # Used when trying to figure out the full scene structure not only a part

    # Setup the center of the world and dimensions
    def setEnvironmentParams(self, dataPath : str, pos_x, pos_y, width, depth, height):
        assert os.path.exists(dataPath), f"The datapath given does not exist"
        ...
        self.depth = depth
        self.height = height
        self.begin_z = 0 # Where should base Z axis read from ? default 0...

        # Setup the camera transforms
        if (self.isCustom is False or self.dataAlreadyInWorldSpace is False) and not self.realtime_data:
            print(" Data is already in world coordinates")
            self.WorldToCameraRotation, self.WorldToCameraTranslation, frames = get_camera_matrix(self.dataPath)
        else:
            self.WorldToCameraRotation = np.eye(3)
            self.WorldToCameraTranslation = np.array(get_translation(self.dataPath, self.realtime_data)).reshape((3, 1))
...
def CreateDefaultDatasetOptions_Waymo(metadata):
    ...
    options.dataAlreadyInWorldSpace = True  # Is data already converted to world space ?
    options.framesIndicesCaptured = list(np.arange(min(0, metadata['frame_min']), metadata['frame_max']))
    options.LIMIT_FRAME_NUMBER = metadata['frame_max']  # How much we want to go with processing...max is default
    options.KEEP_ORIGINAL_FLOATING_POINTS = False
    options.frameSkip = 1  # Not needed on inference or training, just for reconstruction process
    options.numInitialFramesToIgnore = (metadata['frame_min'] - 1)  # Somehow redundant, but it is used in CARLA for example to force ignore first frame...
    options.filter2D = True  # if used, the filter will try to extract noise from the reconstructed environment using a simple median filter
    options.isColmap = False
    options.envSettings = None  # Used only for colmap for now, maybe we should use it uniformly
    options.realtime_data = False
    options.realtime_dataOnline =False

    return options


def CreateDefaultDatasetOptions_CarlaRealTime(metadata, forcedNumFramesPerEpisodes=-1):
    options = DatasetOptions()
    options.isCustom = True

    options.dataAlreadyInWorldSpace = True  # Is data already converted to world space ?
    options.framesIndicesCaptured = list(np.arange(0, (forcedNumFramesPerEpisodes if forcedNumFramesPerEpisodes > 0 else metadata.onlineEnvSettings.numFramesPerEpisode)))
    options.LIMIT_FRAME_NUMBER = 99999999
    #options.KEEP_ORIGINAL_FLOATING_POINTS = False
    options.frameSkip = 1  # Not needed on inference or training, just for reconstruction process
    options.numInitialFramesToIgnore = -1 
    options.filter2D = False # if used, the filter will try to extract noise from the reconstructed environment using a simple median filter
    options.isColmap = False
    options.envSettings = None # Used only for colmap for now, maybe we should use it uniformly
    options.realtime_data=True
    options.realtime_dataOnline = metadata.realTimeEnvOnline if metadata != None else False

    return options

def CreateDefaultDatasetOptions_CarlaOffline(settings):
    options = DatasetOptions()
    options.isCustom = False

    options.dataAlreadyInWorldSpace = True  # Is data already converted to world space ?
    options.framesIndicesCaptured = list(np.arange(0, 500))
    options.LIMIT_FRAME_NUMBER = 99999999
    #options.KEEP_ORIGINAL_FLOATING_POINTS = False
    options.frameSkip = 1  # Not needed on inference or training, just for reconstruction process
    options.numInitialFramesToIgnore = 50
    options.filter2D = False # if used, the filter will try to extract noise from the reconstructed environment using a simple median filter
    options.isColmap = False
    options.envSettings = None # Used only for colmap for now, maybe we should use it uniformly
    options.realtime_data=False
    options.realtime_dataOnline =False
    options.trainableWalkerIds = None
    options.trainableVehiclesIds = None
    options.outPedestrianIndices = None
    options.outCarIndices = None

    return options


def CreateDefaultDatasetOptions_CarlaRealTimeOffline(metadata, forcedNumFramesPerEpisodes=-1):
    options = DatasetOptions()
    options.isCustom = True
    options.dataAlreadyInWorldSpace = True  # Is data already converted to world space ?
    options.framesIndicesCaptured = list(np.arange(0, 3000))
    options.LIMIT_FRAME_NUMBER = 99999999
    #options.KEEP_ORIGINAL_FLOATING_POINTS = False
    options.frameSkip = 1  # Not needed on inference or training, just for reconstruction process
    options.numInitialFramesToIgnore = -1
    ...
    return options
def CreateDefaultDatasetOptions_Cityscapes(settings):
    options = DatasetOptions()
    options.isCustom = True

    options.filter2D = True # if used, the filter will try to extract noise from the reconstructed environment using a simple median filter
    options.LIMIT_FRAME_NUMBER = 99999999
    options.dataAlreadyInWorldSpace = False  # Is data already converted to world space ?
    options.frameSkip = 50 # Why 50?
    options.isColmap = True
    options.envSettings = None
    options.realtime_data =False
    options.realtime_dataOnline =False
    options.isOldDatasetRangeFormat = True
    options.numInitialFramesToIgnore = 1

    return options

def CreateDefaultEmptyDatasetOptions():
    options = DatasetOptions()
    options.isCustom = False
