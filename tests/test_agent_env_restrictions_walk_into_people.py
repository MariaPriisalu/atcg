...
from RL.settings import run_settings,NBR_REWARD_WEIGHTS, PEDESTRIAN_REWARD_INDX,PEDESTRIAN_INITIALIZATION_CODE
...
class TestEnv(unittest.TestCase):
    ...
    #     setup = run_settings()
    #     setup.end_on_bit_by_pedestrians=True
    #     setup.useRLToyCar=False
    #     setup.multiplicative_reward_pedestrian=False
    ...
    #                             end_collide_ped=True, defaultSettings=setup,centering={})
    ...
    def test_walk_into_people(self):
    ...
    def test_walk_into_objs_varied_vel(self):
        ...
        # Valid move
        self.update_agent_and_episode([0, -.7, 0], agent, environmentInteraction, episode, 8)
    ...
    def test_walk_into_objs_wide(self):
        ...
        self.update_agent_and_episode([0, -1, 0], agent, environmentInteraction, episode, 14)
    ...
    def test_walk_into_objs_wide_varied_vel(self):
        ...
        self.update_agent_and_episode([0, 0, 0.7], agent, environmentInteraction, episode, 4)
        ...
        self.update_agent_and_episode([0, 0, 0.8], agent, environmentInteraction, episode, 6)
        ...
        self.update_agent_and_episode([0, -.7, 0], agent, environmentInteraction, episode, 7)
        # agent.perform_action([0, -.7, 0], episode)
        # agent.update_agent_pos_in_episode(episode, 7)
        # agent.on_post_tick(episode)
        # agent.update_metrics(episode)
        ...
        np.testing.assert_array_equal(agent.position, [1, 4, 7])
        np.testing.assert_array_equal(episode.calculate_reward(11, episode_done=True)[0], 0)
        self.update_agent_and_episode([0, -.7, 0], agent, environmentInteraction, episode, 14)
        np.testing.assert_array_equal(episode.calculate_reward(12, episode_done=True)[0], 0)
    ...
    def test_pedestrians(self):
        ...
        people_dict={0:people_list1, 1:people_list2, 3:people_list3}
        init_frames={0:0, 1:0, 3:0}
        rewards = self.get_reward(True)
        rewards[PEDESTRIAN_REWARD_INDX.collision_with_pedestrian] = -1
        agent, episode, environmentInteraction  = self.initialize_episode(cars, gamma, people, pos_x, pos_y, tensor, seq_len=7,  rewards=rewards, people_dict=people_dict, init_frames=init_frames)
        episode.agent_size = [0, 0, 0]
        ...
        # agent.update_metrics(episode)
        # episode.pedestrian_data[0].action[2] = 5
        np.testing.assert_array_equal(episode.pedestrian_data[0].agent[3], [0, 3, 3])
        ...
        # agent.update_metrics(episode)
        # episode.pedestrian_data[0].action[3] = 5
        np.testing.assert_array_equal(episode.pedestrian_data[0].agent[4], [0, 2,2])
        np.testing.assert_array_equal(episode.calculate_reward(2, episode_done=True)[0], 0)
        ...
        # agent.update_metrics(episode)
        # episode.pedestrian_data[0].action[4] = 5
        np.testing.assert_array_equal(episode.pedestrian_data[0].agent[5], [0, 2, 3])
        ...
        # agent.update_metrics(episode)
        # episode.pedestrian_data[0].action[5] = 5
        np.testing.assert_array_equal(episode.pedestrian_data[0].agent[6], [0, 2, 4])
        np.testing.assert_array_equal(episode.calculate_reward(4, episode_done=True)[0], 0)
        self.update_agent_and_episode([0, 0, 1], agent, environmentInteraction, episode, 7, breadth=[0, 1, 1])
        np.testing.assert_array_equal(episode.calculate_reward(5, episode_done=True)[0], 0)
    ...
    def test_pedestrians_varied_vel(self):
        ...
        # agent.update_metrics(episode)
        # episode.pedestrian_data[0].action[1] = 5
        np.testing.assert_array_almost_equal(episode.pedestrian_data[0].agent[2], [0,2, 2.1])
        #np.testing.assert_array_equal(episode.pedestrian_data[0].agent[2], [0, 2, 2])
        np.testing.assert_array_equal(episode.calculate_reward(0, episode_done=True)[0], -1)
        ...
        np.testing.assert_array_almost_equal(episode.pedestrian_data[0].agent[6], [0, .2, 1.95])
        np.testing.assert_array_equal(episode.calculate_reward(4, episode_done=True)[0], 0)
        self.update_agent_and_episode([0, 0, .75], agent, environmentInteraction, episode, 7)
        np.testing.assert_array_equal(episode.calculate_reward(5, episode_done=True)[0], 0)
    ...
    def test_pedestrians_wide(self):
        ...
        rewards[PEDESTRIAN_REWARD_INDX.on_pedestrian_trajectory] = 1
        agent, episode , environmentInteraction = self.initialize_episode(cars, gamma, people, pos_x, pos_y, tensor, seq_len=14, rewards=rewards, people_dict=people_dict, init_frames=init_frames, agent_size=[0,1,1])
        episode.agent_size = [0, 0, 0]
        episode.initial_position(0,None)
        episode.pedestrian_data[0].agent[0] = [0, 0, 0]
        ...
        agent, episode, environmentInteraction = self.initialize_episode(cars, gamma, people, pos_x, pos_y, tensor,
                                                                         ...
                                                                         people_dict=people_dict,
                                                                         init_frames=init_frames, agent_size=[0, 1, 1])
        episode.agent_size = [0, 0, 0]
        episode.initial_position(0, None)
        episode.pedestrian_data[0].agent[0] = [0, 0, 0]
        episode.initial_position(0, None)
    ...
    def test_cars_hit_on_one(self):
        ...
        car_list.append([0, 2 + 1, 0, 0 + 1, 1, 1 + 1])

        cars[2].append([0, 2 + 1, 1, 1 + 1, 1, 1 + 1])
        car_list.append([0, 2 + 1, 1, 1 + 1, 1, 1 + 1])

        cars[3].append([0, 2 + 1, 2, 2 + 1, 1, 1 + 1])
        car_list.append([0, 2 + 1, 2, 2 + 1, 1, 1 + 1])
        cars_dict={0:car_list}
        cars_init={0:1}
        tensor=np.ones(tensor.shape)*cityscapes_labels_dict['sidewalk']/NUM_SEM_CLASSES
        rewards = self.get_reward(True)
        rewards[PEDESTRIAN_REWARD_INDX.collision_with_car] = -1
        agent, episode, environmentInteraction  = self.initialize_episode(cars, gamma, people, pos_x, pos_y, tensor, seq_len=seq_len, rewards=rewards, car_dict=cars_dict, init_frames_cars=cars_init)
        episode.agent_size = [0, 0, 0]
        episode.initial_position(0,None)
    ...
    def test_cars_follow_car_vel(self):
        ...
        agent, episode, environmentInteraction  = self.initialize_episode(cars, gamma, people, pos_x, pos_y, tensor, seq_len=seq_len,
                                                 rewards=rewards, car_dict=cars_dict, init_frames_cars=cars_init)
        episode.agent_size = [0, 0, 0]
        episode.initial_position(0,None)
        episode.pedestrian_data[0].agent[0] = np.array([0, 0, 0])
    ...
    def test_follow_agent_reward(self):
        ...
        print(episode.pedestrian_data[0].goal_person_id_val)
    ...
    def test_mov_penalty_reward(self):
        ...
        for frame in range(4, 8):
            ...
            np.testing.assert_array_equal(episode.pedestrian_data[0].agent[frame+1], [0, frame+1-4,4 ])
            if frame > 1:
                np.testing.assert_approx_equal(episode.calculate_reward(frame, episode_done=True)[0], 0)
                np.testing.assert_approx_equal(episode.pedestrian_data[0].measures[frame, PEDESTRIAN_MEASURES_INDX.change_in_direction], 0)
    ...
    def test_mov_penalty_reward_two(self):
        ...
        self.update_agent_and_episode([0, 1, 1], agent, environmentInteraction, episode, 13)
        np.testing.assert_approx_equal(episode.calculate_reward(11, episode_done=True)[0],0)
    ...
    def test_mov_penalty_reward_three(self):
        ...
        self.update_agent_and_episode([0, 1, 1], agent, environmentInteraction, episode, 12, action_nbr=8)
        np.testing.assert_approx_equal(episode.calculate_reward(10, episode_done=True)[0], -np.sin(np.pi/8))
