...
try:
	sys.path.append(glob.glob(os.path.join(CARLA_INSTALL_PATH, 'PythonAPI/carla/dist/carla-*%d.%d-%s.egg') % (
		sys.version_info.major,
		sys.version_info.minor,
		'win-amd64' if os.name == 'nt' else 'linux-x86_64'))[0])
	print("Found Path " + str(os.path.join(CARLA_INSTALL_PATH, 'PythonAPI/carla/dist/carla-*%d.%d-%s.egg')))
except IndexError:
	assert "Path to carla egg not found !"



sys.path.append(os.path.join(CARLA_INSTALL_PATH, 'PythonAPI/carla'))
print ("Path "+str(os.path.join(CARLA_INSTALL_PATH, 'PythonAPI/carla')) )
import carla
print( 'PATH to second CARLA import ')
print(dir(carla))
