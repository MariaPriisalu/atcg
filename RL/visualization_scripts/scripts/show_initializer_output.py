...
import os
from RL.visualization_scripts.read_files_utils import read_settings_file
from RL.visualization_scripts.find_files_utils import sort_files_eval
from utils.constants import Constants
from RL.settings import run_settings
from RL.environment_waymo import CARLAEnvironment
from RL.visualization import view_2D, view_pedestrians, view_cars, view_valid_pos,view_2D_rgb,view_pedestrians_nicer,view_cars_nicer,view_agent_nicer, view_valid_init_pos, view_valid_pedestrians, view_agent, view_agent_input,plot_car
...
timestamps=["2021-09-15-10-01-32.036798"]
ending=''

calc_dist=True
folder_name="/adv_initializer_output/"
################################################################################3
settings_path="some_results/"
settings_ending="settings.txt"
stat_path = "Datasets/cityscapes/pointClouds/train/"
...
for timestamp in timestamps:
    ...
    rl = RL()
    counter, filecounter, images_path, init, net, saver, tmp_net, init_net, goal_net, car_net,init_car_net = rl.pre_run_setup(
        settings)
...
if len(test_files)>0:
        ...
        for ep_itr, scene in enumerate([scenes]):
            ...
            map_path = filepath_npy[:-len(".npy")] + "_init_map.npy"
            ...
            if nbr!=prev_nbr:
                ...
                if True:
                    ...
                    pos_x=0
                    pos_y=-64
                    ep = env.set_up_episode(get_carla_prefix_eval( False, True,False, False), file_name, pos_x, pos_y, training, evaluate=True, useCaching=settings.useCaching, seq_len_pfnn=seq_len_pfnn,datasetOptions=datasetOptions, car=car, no_external=settings.ignore_external_cars_and_pedestrians)
                    ...
                    print("View PEDESTRIANS")
                    img3 = view_pedestrians_nicer(0, ep.people, img3, trans=.6 )  # tensor=ep.reconstruction)
                    #current_frame, cars, frame,

                    img3 = view_cars_nicer(img3,ep.cars, 0)  # ,tensor=ep.reconstruction)


            if "test_175" in basename and epoch==9:
                cur_stat=scene[0]
                ...
                if ep_itr>=len(car_stat.shape[0]):
                    ep_itr=0
                car_poses = car_stat[ep_itr, :, STATISTICS_INDX_CAR.bbox[0]:STATISTICS_INDX_CAR.bbox[1]]
                heat_maps = np.load(map_path)
                prior=heat_maps[ep_itr,:,STATISTICS_INDX_MAP.prior]
                init_distr = heat_maps[ep_itr, :, STATISTICS_INDX_MAP.init_distribution]
                distr=prior*init_distr
                prior=prior.reshape(img_from_above.shape[:2])
                distr = distr.reshape(img_from_above.shape[:2])
                ...
                img_h = plt.imshow(np.flip(rescaled_prob, axis=0), alpha=0.6, cmap='plasma')
                if counter == 0:
                    right_cb = fig.colorbar(img_h, fraction=0.02)
                    counter = 1
                plt.axis('off')
                plt.tight_layout()
                print(save_images_path+folder_name + basename+"_" +str(timestamp)+ "_prior_rgb_" + str(epoch)+".png")
                fig.savefig(save_images_path+folder_name + basename+"_" +str(timestamp)+ "_prior_rgb_" + str(epoch)+".png", bbox_inches='tight')

                plt.imshow(img_from_above_loc_rgb)
                maxval = np.max(distr[:])
                print(" max val of goal " + str(maxval))
                rescaled_prob = distr / maxval
                img_h = plt.imshow(np.flip(rescaled_prob, axis=0), alpha=0.6, cmap='plasma')
                if counter == 0:
                    right_cb = fig.colorbar(img_h, fraction=0.02)
                    counter = 1
                ...
                plt.tight_layout()
                print(save_images_path+folder_name + basename+"_" +str(timestamp)+ "_prior_rgb_" + str(epoch)+".png")
                fig.savefig(save_images_path + folder_name + basename + "_" + str(timestamp) + "_distr_rgb_" + str(
                ...
                plt.tight_layout()
                print(
                    save_images_path + folder_name + basename + "_" + str(timestamp) + "_prior_" + str(epoch) + ".png")
                fig.savefig(
                    save_images_path + folder_name + basename + "_" + str(timestamp) + "_prior_" + str(epoch) + ".png",
                    bbox_inches='tight')
                ...
                print(
                    save_images_path + folder_name + basename + "_" + str(timestamp) + "_prior_" + str(epoch) + ".png")
                fig.savefig(save_images_path + folder_name + basename + "_" + str(timestamp) + "_distr_" + str(
                    epoch) + ".png", bbox_inches='tight')
