...
class Seg_2d(Net_2d):
    ...
    def get_input(self, id,episode, agent_pos_cur, frame_in=-1, training=True):
        ...
        tensor, people, cars, people_cv, cars_cv = episode.get_agent_neighbourhood(id,agent_pos_cur, [self.settings.agent_s[1], self.settings.agent_s[2]],frame_in, pedestrian_view_occluded=self.settings.pedestrian_view_occluded, field_of_view=self.settings.field_of_view)
...
class Seg_2d_min(Net_2d):
    ...
    def get_input(self,id, episode, agent_pos_cur, frame_in=-1, training=True):
        ...
        tensor, people, cars, people_cv, cars_cv  = episode.get_agent_neighbourhood(id,agent_pos_cur, self.settings.agent_s, frame_in, pedestrian_view_occluded=self.settings.pedestrian_view_occluded, field_of_view=self.settings.field_of_view)
