...
class  GaussianNet(SoftMaxNet):
    ...
    def define_loss(self, dim_p):
        ...
        if self.settings.sigma_vel<0:
            ...
            self.normal_dist = tfd.Normal(self.mu_vel, self.sigma_vel)
            self.responsible_output = self.normal_dist.prob(self.sample)
            self.loss=0.5 * tf.reduce_sum(input_tensor=tf.square((self.sample - self.mu_vel) / self.sigma_vel), axis=-1) \
            + 0.5 * np.log(2.0 * np.pi) * tf.cast(tf.shape(input=self.sample)[-1], dtype=tf.float32) + tf.reduce_sum(input_tensor=self.logstd, axis=-1)
            self.loss=self.loss*self.advantages
            self.loss=self.loss-(1e-1* tf.reduce_sum(input_tensor=self.logstd + .5 * np.log(2.0 * np.pi * np.e), axis=-1))
        else:
            self.normal_dist = tfd.Normal(self.mu_vel, self.settings.sigma_vel)
            self.responsible_output = self.normal_dist.prob(self.sample)
            ...
            self.loss = tf.reduce_mean(input_tensor=self.advantages*(2*tf.math.log(self.settings.sigma_vel)+tf.math.log(2*pi)+(self.l2_loss/(self.settings.sigma_vel*self.settings.sigma_vel))))
    ...
    def apply_net(self,id, feed_dict, episode, frame, training, max_val=False, viz=False, manual=False):
        ...
        if self.settings.sigma_vel > 0:
            ...
            mean_vel, sigma, summary_train, log_sigma = self.sess.run([self.mu_vel, self.sigma_vel, self.train_summaries, self.logstd],feed_dict)
        ...
        self.take_step(id,episode, frame, max_val, mean_vel, sigma, training)

        return episode.pedestrian_data[id].velocity[frame]

    def take_step(self, id, episode, frame, max_val, mean_vel, sigma, training):

        if episode.pedestrian_data[id].goal_person_id >= 0 and frame + 1 < len(episode.valid_people_tracks[
                                                              episode.pedestrian_data[id].goal_person_id_val]) and training:  # np.linalg.norm(self.actions[value]) >0:
            ...
            episode.pedestrian_data[id].velocity[frame] = np.mean(episode.valid_people_tracks[episode.pedestrian_data[id].goal_person_id_val][next_frame] -
                                              episode.valid_people_tracks[episode.pedestrian_data[id].goal_person_id_val][current_frame],
                                              axis=1)
            ...
            episode.pedestrian_data[id].probabilities[frame, 0:2] = np.copy(mean_vel)
            episode.pedestrian_data[id].probabilities[frame, 2:4] = np.copy(sigma)
            # print "Follow pedestrian action: " + str(episode.pedestrian_data[self.id].action[frame]) + " " + str(episode.velocity[frame])
            ...
            episode.pedestrian_data[id].velocity[frame] = np.zeros(3)
            if speed_value > 1e-3:
                default_speed = 3.0
            ...
            episode.pedestrian_data[id].velocity[frame][1] = copy.copy(speed_value_y[0] * 5 / episode.frame_rate)
            episode.pedestrian_data[id].velocity[frame][2] = copy.copy(speed_value_z[0] * 5 / episode.frame_rate)
            episode.pedestrian_data[id].speed[frame] = speed_value * 5 / episode.frame_rate
    ...
    def calc_probabilities(self, fc_size):
        if not self.settings.old_fc:
            ...
            # else:
            #     self.fc_out = self.mu
            weights = tf.compat.v1.get_variable('weights_last', [9, 2], self.DTYPE,
    ...
    #                                  episode, feed_dict, frame, poses, statistics, training):
    ...
    #
    # def get_feature_vectors(self, agent_frame, episode, feed_dict, frame, distracted=False):
    #     dim_p = self.get_dim_p()
    #     action_size = 2
    #     super(GaussianNet, self).get_feature_vectors(agent_frame, episode, feed_dict, frame,distracted)
    #     values = np.zeros(action_size * self.settings.action_mem)
    #     for past_frame in range(1, self.settings.action_mem + 1):
    #         if agent_frame - past_frame >= 0:
    #
    ...
    #                 max(agent_frame - past_frame, 0), episode)
    #         else:
    #
    #             values[(past_frame - 1) * action_size] =episode.vel_init[1]
    #             values[(past_frame - 1) * action_size+1] = episode.vel_init[2]
    #
    #     feed_dict[self.action_mem] = np.expand_dims(values, axis=0)
    #     if self.settings.nbr_timesteps > 0:
    #         values = np.zeros(action_size * self.settings.nbr_timesteps)
    ...
    #             else:
    #
    #                 values[(past_frame - 2) * action_size] = episode.vel_init[1]
    #                 values[(past_frame - 2) * action_size+1] = episode.vel_init[2]
    #         feed_dict[self.action_mem_further] = np.expand_dims(values, axis=0)
    #
    #
    # def get_memory_vel2_episode(self, frame, episode):
    #     return episode.velocity[frame][2]
    #
    # def get_memory_vel1_episode(self, frame, episode):
    #     return episode.velocity[frame][1]

    def get_sample(self,id, statistics, ep_itr, agent_frame):
        frameRate, frameTime = self.settings.getFrameRateAndTime()
        return statistics[ep_itr,id, agent_frame, 4:6] * frameRate / 5.0
...
class Angular_Net(GaussianNet):
    def define_loss(self, dim_p):
        ...
        self.normal_dist = tfd.Normal(self.mu_vel, self.settings.sigma_vel)
        self.responsible_output = self.normal_dist.prob(self.sample)
        self.loss = -tf.reduce_sum(input_tensor=(self.normal_dist.log_prob(self.sample) * self.advantages))
        self.loss -= 1e-1 * tf.reduce_sum(input_tensor=self.normal_dist.entropy())
    ...
    def take_step(self,id,  episode, frame, max_val, mean_vel, sigma, training):
        ...
        episode.pedestrian_data[id].speed[frame] = speed_value * 5 / episode.frame_rate
        episode.pedestrian_data[id].action[frame] = angle
        episode.pedestrian_data[id].velocity[frame] = np.zeros(3)
        episode.pedestrian_data[id].velocity[frame][1] = copy.copy(episode.speed[frame] * np.cos(angle))
        episode.pedestrian_data[id].velocity[frame][2] = copy.copy(-episode.speed[frame] * np.sin(angle))
        # print "Action "+str(episode.velocity[frame])
        episode.pedestrian_data[id].action[frame] = 0
        episode.pedestrian_data[id].probabilities[frame, 0:2] = np.copy(mean_vel)
        episode.pedestrian_data[id].probabilities[frame, 2:4] = np.copy(sigma)
    ...
    def get_sample(self,id, statistics, ep_itr, agent_frame):
        frameRate, frameTime = self.settings.getFrameRateAndTime()
        sample = copy.copy(statistics[ep_itr,id, agent_frame, 4:6] * frameRate / 5.0) / 3.0
        sample[0]=np.sqrt(statistics[ep_itr,id, agent_frame, 4]**2+ statistics[ep_itr,id, agent_frame, 5]**2)
        sample[1]=statistics[ep_itr,id, agent_frame, 6]*2/np.pi#2/np.pi
        return sample

    # def get_memory_vel2_episode(self, agent_frame, episode, past_frame):
    #     return (episode.pedestrian_data[self.id].action[max(agent_frame - past_frame, 0), 6] )* 2/ np.pi
    #
    # def get_memory_vel1_episode(self, agent_frame, episode, past_frame):
    #     return (episode.speed[max( agent_frame - past_frame, 0)] * 17.0 / 5.0 ) / 3.0
    #
    #
    ...
    def get_memory_vel2_episode(self, frame, episode):
        if len(episode.agent[frame + 1]) > 0:
            # print "Frame "+str(frame +1 )+" "+str(episode.agent[frame+1])+" "+str(episode.agent[frame])
            # print " 2nd: "+str(episode.agent[frame+1]-episode.agent[frame])+" "+str(episode.agent[frame+1][2]-episode.agent[frame][2])
            return (episode.agent[frame+1][2]-episode.agent[frame][2])
        return 0

    def get_memory_vel1_episode(self, frame, episode):
        if len(episode.agent[frame + 1])>0:
            # print "Frame " + str(frame + 1) + " " + str(episode.agent[frame + 1]) + " " + str(
            #     episode.agent[frame])
            # print " 1st: " + str(episode.agent[frame + 1] - episode.agent[frame]) + " " + str(
            #    episode.agent[frame + 1][1] - episode.agent[frame][1])
            return ( episode.agent[frame + 1][1]-episode.agent[frame][1])
        return 0
    ...
    def get_feature_vectors_gradient(self,id, agent_action, agent_frame, agent_measures, agent_pos, agent_speed,agent_velocity, ep_itr,
        ...
        super(Angular_Net, self).get_feature_vectors_gradient(id,agent_action, agent_frame, agent_measures, agent_pos, agent_speed,agent_velocity, ep_itr,
                                     episode, feed_dict, frame, poses, statistics, training,statistics_car)
        feed_dict[self.angle]=np.expand_dims(np.array([statistics[ep_itr,id, max(agent_frame, 0), int(NUM_SEM_CLASSES)]/np.pi]), axis=0)
    ...
    def get_feature_vectors(self, id, agent_frame, episode, feed_dict, frame):
        super(Angular_Net, self).get_feature_vectors(id, agent_frame, episode, feed_dict, frame)
        rotation_matrix = np.identity(2)
        feed_dict[self.angle] = np.expand_dims(np.array([episode.pedestrian_data[id].angle[agent_frame] / np.pi]), axis=0)
        # print "Angle "+str(episode.angle[agent_frame])+" "+str(agent_frame)
        # rotation_matrix[0, 0] = np.cos(episode.angle[agent_frame])
        # rotation_matrix[1, 1] = np.cos(episode.angle[agent_frame])
        # rotation_matrix[0, 1] = -np.sin(episode.angle[agent_frame])
        # rotation_matrix[1, 0] = np.sin(episode.angle[agent_frame])
        # print rotation_matrix
        # if self.settings.car_var:
        #     #print "car var before rotation " +str(feed_dict[self.cars]* np.sqrt(episode.reconstruction.shape[1] ** 2 + episode.reconstruction.shape[2] ** 2))
        #     feed_dict[self.cars] = np.transpose(np.matmul(rotation_matrix, np.transpose(np.array(feed_dict[self.cars]))))
        #     #print "car var after rotation: "+str(feed_dict[self.cars]* np.sqrt(episode.reconstruction.shape[1] ** 2 + episode.reconstruction.shape[2] ** 2))
        # if self.settings.goal_dir:
        #     #print "goal before rotation " + str(feed_dict[self.goal_dir]* np.sqrt(episode.reconstruction.shape[1] ** 2 + episode.reconstruction.shape[2] ** 2))
        #     feed_dict[self.goal_dir] = np.transpose(np.matmul(rotation_matrix, np.transpose(np.array(feed_dict[self.goal_dir]))))
        #     #print "goal after rotation " + str(feed_dict[self.goal_dir]* np.sqrt(episode.reconstruction.shape[1] ** 2 + episode.reconstruction.shape[2] ** 2))
...
class Angular_No_vel_Net(Angular_Net):
    ...
    def apply_net(self, id, feed_dict, episode, frame, training, max_val=False, viz=False,manual=False):
        ...
        episode.pedestrian_data[id].velocity[frame][1] = copy.copy(episode.pedestrian_data[id].speed[frame]*np.cos(angle))
        episode.pedestrian_data[id].velocity[frame][2] = copy.copy(-episode.pedestrian_data[id].speed[frame]*np.sin(angle))
        print(("Action "+str(episode.velocity[frame])))
        episode.pedestrian_data[id].action[frame] = episode.find_action_to_direction(episode.pedestrian_data[id].velocity[frame], episode.pedestrian_data[id].speed[frame] )
        episode.pedestrian_data[id].probabilities[frame, 0:2] = np.copy(mean_vel)
        episode.pedestrian_data[id].probabilities[frame, 3] = np.copy(self.settings.sigma_vel)
...
class ContinousMemNet(GaussianNet):
    ...
    def get_feature_vectors(self,id, agent_frame, episode, feed_dict, frame):
        super(ContinousMemNet, self).get_feature_vectors(id, agent_frame, episode, feed_dict, frame)
        # if episode.goal_person_id>0:
        #     #print str(len(episode.valid_people_tracks[episode.goal_person_id]))+" "+str(agent_frame)+" "+str(frame)
        #     agent_pos = np.mean(episode.valid_people_tracks[episode.goal_person_id][agent_frame], axis=1)
        dim_p = self.get_dim_p()
        ...
        if self.settings.action_mem:
            ...
            for past_frame in range(1, self.settings.action_mem+1):
                if agent_frame - past_frame >= 0:
                    values[(past_frame - 1) * action_size] = self.get_memory_vel1_episode(id, max(agent_frame - past_frame, 0), episode)
                    values[(past_frame - 1) * action_size + 1] = self.get_memory_vel2_episode(id, max(agent_frame - past_frame, 0), episode)
            ...
            if self.settings.nbr_timesteps > 0:
                ...
                for past_frame in range(self.settings.action_mem + 1, self.settings.nbr_timesteps + 2):
                    if agent_frame - past_frame >= 0:
                        ...
                        values[(past_frame - 2) * action_size] = self.get_memory_vel1_episode(id,max(agent_frame - past_frame, 0), episode)
                        values[(past_frame - 2) * action_size + 1] = self.get_memory_vel2_episode(id,max(agent_frame - past_frame, 0), episode)
                feed_dict[self.action_mem_further] = np.expand_dims(values, axis=0)



    def get_memory_vel2_episode(self, id,frame, episode):

        return episode.pedestrian_data[id].agent[min(frame + 1, len(episode.agent)-1)][2] - episode.agent[frame][2]

    def get_memory_vel1_episode(self,id, frame, episode):

        return episode.pedestrian_data[id].agent[min(frame+1, len(episode.agent)-1)][1]-episode.agent[frame][1]


    def get_feature_vectors_gradient(self, id, agent_action, agent_frame, agent_measures, agent_pos, agent_speed,agent_velocity, ep_itr,
                                                          episode, feed_dict, frame, poses, statistics, training, statistics_car=[]):
        dim_p = self.get_dim_p()
        super(ContinousMemNet, self).get_feature_vectors_gradient(id, agent_action, agent_frame, agent_measures, agent_pos, agent_speed,agent_velocity, ep_itr,
                                                          episode, feed_dict, frame, poses, statistics, training,statistics_car)
        if self.settings.action_mem:
            #print dim_p
            ...
            for past_frame in range(1, self.settings.action_mem+1):
                if agent_frame - past_frame >= 0:
                        ...
                        values[(past_frame - 1) * dim_p] = self.get_memory_vel1( id, max(agent_frame - past_frame, 0),
                                                                                                       ep_itr,
                                                                                                       statistics)
                        values[(past_frame - 1) * dim_p + 1] = self.get_memory_vel2(id,
                         max(agent_frame - past_frame, 0), ep_itr, statistics)
            ...
            if self.settings.nbr_timesteps > 0:
                ...
                for past_frame in range(self.settings.action_mem+1, self.settings.nbr_timesteps + 1):
                    if agent_frame - past_frame >= 0:
                        ...
                        values[(past_frame - 2) * dim_p] = self.get_memory_vel1(id,
                             max(agent_frame - past_frame, 0),
                             ep_itr,
                             statistics)
                        values[(past_frame - 2) * dim_p + 1] = self.get_memory_vel2(id,
                            max(agent_frame - past_frame, 0), ep_itr, statistics)
                feed_dict[self.action_mem_further] = np.expand_dims(values, axis=0)

    def get_memory_vel2(self,id, frame, ep_itr, statistics):
        ...
        return statistics[ep_itr,id, min(frame+1,statistics.shape[1]-1 ), 2]-statistics[ep_itr, frame, 2]

    def get_memory_vel1(self,id, frame, ep_itr,statistics):
        ...
        return statistics[ep_itr,id, min(frame+1,statistics.shape[1] -1), 1] - statistics[ep_itr, frame, 1]
