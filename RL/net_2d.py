...
class Net_2d(Net):
    ...
    def define_conv_layers(self):
        ...
        with tf.compat.v1.variable_scope('conv1') as scope:
            ...
            kernel1 = tf.compat.v1.get_variable('weights', [ 3, 3, self.nbr_channels, out_filters], self.DTYPE,
                                      initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))

            self.conv_out1 = tf.nn.conv2d(input=prev_layer, filters=kernel1, strides=[1, 1, 1, 1], padding='SAME')  # [1, 2, 2, 2, 1]
        ...
        if 1 in self.settings.pooling:
            pooled = tf.nn.max_pool2d(input=prev_layer, ksize=[1, 2, 2, 1], strides=[1, 1, 1, 1],padding='SAME')#, 'SAME')
        ...
        if self.settings.num_layers==2:
            with tf.compat.v1.variable_scope('conv2') as scope:
                ...
                self.conv_out2 = tf.nn.conv2d(input=prev_layer, filters=kernel2, strides=[1, 1, 1, 1], padding='SAME')  # [1, 2, 2, 2, 1]
                if self.settings.conv_bias:
        ...
        if 2 in self.settings.pooling:
            pooled2 = tf.nn.max_pool2d(input=prev_layer, ksize=[1, 2, 2, 1], strides=[1, 1, 1, 1], padding='SAME')#, 'SAME')
    ...
    def get_input(self,id, episode, agent_pos_cur, frame_in=-1,training=True):
        tensor,people,cars, people_cv, cars_cv= episode.get_agent_neighbourhood(id,agent_pos_cur, self.settings.agent_s, frame_in, pedestrian_view_occluded=self.settings.pedestrian_view_occluded, field_of_view=self.settings.field_of_view) # TODO!
