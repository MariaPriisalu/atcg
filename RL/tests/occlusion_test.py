...
from utils.utils_functions import getBordersImg,getOcclusionImg
...
def draw(car_pos):
    ...
    occlusion_map = np.zeros_like(initial_img)
    validImg = np.ones_like(base_img) - base_img
    occlusion_map = getOcclusionImg(occlusion_map, initial_img, car_pos, 1, validImg)
    binstruct = ndimage.generate_binary_structure(2, 2)
    occlusion_map = ndimage.binary_closing(occlusion_map, binstruct)
    plt.subplot(133)
    plt.imshow(occlusion_map, cmap=plt.cm.gray)
    print("checkA")
...
if __name__ == "__main__":
    ...
    for i in range(5):
        car_pos=np.array([np.random.randint(0,512),np.random.randint(0,512)])
        print(car_pos)
        draw(car_pos)