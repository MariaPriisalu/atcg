...
from commonUtils.ReconstructionUtils import NUM_SEM_CLASSES, NUM_SEM_CLASSES_ASINT
            ...
            if indx%100==0:
                for train_nbr in self.train_nbrs:
                    statistics, people, cars, recon, episode = self.act_and_learn(agent, None, path + "_" + str(train_nbr) + "_" + str(itr),
                                                                                  ...
                                                                                  False, road_width)
                    self.counter=make_movie(recon,people,cars,  [statistics], self.width, self.depth, self.seq_len_train, self.counter,
                                            self.agent_shape, self.agent_sight, poses, training=True,  number_of_agents=self.settings.number_of_agents, name_movie= self.name_movie, path_results=self.img_dir)
                for train_nbr in self.test_nbrs:
                    ...
                    self.counter=make_movie(recon,people,cars, [statistics], self.width, self.depth, self.seq_len_test, self.counter,
                                            self.agent_shape, self.agent_sight, poses, training=False, number_of_agents=self.settings.number_of_agents,  nbr_episodes_to_vis=5,
    ...
    def set_up_episode(self, cars, people, pos_x, pos_y, tensor, training, road_width=0):
        ...
        if pos_x==0: # horizontal line
            ...
            return SimpleEpisode(self.reconstruction, people, cars, pos_x, pos_y, self.gamma, seq_len,
                                 reward_weights_pedestrian=self.r_weights,reward_weights_initializer=self.r_weights, defaultSettings=self.settings)
            ...
            #np.array([[0, 8], [point[0] - 1, point[0] - 1], [point[1] - 1, point[1] - 1]]))
            return SimpleEpisode(self.reconstruction, people, cars, pos_x, pos_y, self.gamma, seq_len,
                                 reward_weights_pedestrian=self.r_weights,reward_weights_initializer=self.r_weights, defaultSettings=self.settings,centering={})
        elif pos_x == 8:
            for frame in range(seq_len):
                cars.append([])
                people.append([])
