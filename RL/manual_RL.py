...
import sys, os
print("ADD TO path "+str(os.path.dirname( os.path.abspath(__file__)))+" --------------------------------")
...
class RL_manual_no_tf(RL_manual):
    ...
    def run(self,settings, time_file=None, supervised=False):
        ...
        counter, filecounter, images_path, init, net, saver, tmp_net, init_net, goal_net, car_net,init_car_net = self.pre_run_setup(
            settings)
        ...
        if settings.useRLToyCar:
            car = [CarAgent(settings, car_net, grad_buffer_car, init_net=init_car_net, init_grad_buffer=grad_buffer_init_car)]
        else:
            car = None
