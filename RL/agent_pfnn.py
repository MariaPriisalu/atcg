...
class AgentPFNN(SimplifiedAgent):
    ...
    def initial_position(self, pos, goal, current_frame=0, vel=0, init_dir=[],episode=None):
        super(AgentPFNN, self).initial_position(pos, goal, current_frame=current_frame, vel=vel,episode=episode)
        ...
        self.itr_counter = 0
        self.set_init_matrix(pos, episode)
        #print ("Agent position in PFNN agent class: "+str(self.position)+" position "+str(pos)+" "+str(goal))
    ...
    def set_init_matrix(self, next_pos, episode):
        ...
        if np.linalg.norm(self.rotation_matrix) == 0:
            ...
            dir_y = episode.pedestrian_data[self.id].vel_init[1] / self.deltaTime_inMySimulation
            dir_z = episode.pedestrian_data[self.id].vel_init[2] / self.deltaTime_inMySimulation

            if self.settings.printdebug_network_input:
                print ("Set init matrix "+str(self.id)+" rotation matrix for init vel "+str(dir_y)+" "+str(dir_z)+" dir to goal "+str( episode.pedestrian_data[self.id].goal[0,:]-episode.pedestrian_data[self.id].agent[0]))
            ...
            if  distInCm > 122: # compare in cm / second
                ...
                while True:
                    ...
                    if velocityDistance < .5 or (counter >= 20 and velocityDistance < 10) or counter>self.max_frames_to_init_pfnnvelocity: # velocity threshold is now in cm per second that's why it is increased, but let the system start up a bit in the beggining anyway
                        #print("Stopped with velocity error "+str(velocityDistance)+" and counter "+str(counter))
                        break
                    ...
                    while remainingDeltaTime > 0:
                        ...
                        self.pos_pfnn_x = x
                        self.pos_pfnn_z = z
                        # print ("Agent pos in PFNN: x "+str(self.pos_pfnn_x)+" z "+str(self.pos_pfnn_z))
                ...
                # print("Agent speed " + str(speedInThisTick))

            episode.pedestrian_data[self.id].agent_high_frq_pos[self.itr_counter, 0] = copy.copy(x)
            episode.pedestrian_data[self.id].agent_high_frq_pos[self.itr_counter, 1] = copy.copy(z)
            episode.pedestrian_data[self.id].avg_speed[self.itr_counter] = copy.copy(speed)

            self.pos_pfnn_x_init = self.pos_pfnn_x
            self.pos_pfnn_z_init = self.pos_pfnn_z

            episode.pedestrian_data[self.id].agent_pose[self.frame, :] = self.PFNN.getCurrentPose()*self.skeleton_scaling_pfnn_to_carla
            episode.pedestrian_data[self.id].agent_pose_hidden[self.frame] = self.PFNN.getPrevFrame_SecondHiddenLayerOutput()

            episode.pedestrian_data[self.id].agent_pose_hidden[self.frame][np.isnan(episode.pedestrian_data[self.id].agent_pose_hidden[self.frame])]=0
            # print("Set init matrix PFNN agent Hidden pose of frame " + str(self.frame) + " " + str(
            #     np.sum(np.abs(episode.agent_pose_hidden[self.frame]))))
    ...
    def take_step(self, next_pos, vel, episode, force_nextPos=False):
        #print ("Take step PFNN "+str(next_pos))
        self.set_init_matrix(next_pos, episode)
        self.pos_exact = next_pos
        self.position = np.round(self.pos_exact).astype(int)
        return next_pos
    ...
    def get_pfnn_target(self, step):
        ...
        # print ("PFNN target from current pos x "+str(true_targetPos2D_x+ self.pos_pfnn_x)+" z: "+str(true_targetPos2D_z+ self.pos_pfnn_z))
    ...
    def carry_out_step(self,next_pos, step, episode):
        #print ("Carry out step PFNN ")
        if self.settings.realTimeEnvOnline:
            pedestrianLastFrameAction = episode.environmentInteraction.getAgentLastFrameData(self)
            pedestrianLastFrameAction.change_in_pose = 0.0
        ...
        # print ("True PFNN target in CARLA: "+str(target_in_carla)+" pfnn target in pfnn "+str(true_targetPos2D_x)+" "+str(true_targetPos2D_z)+" current pfnn pos "+str(self.pos_pfnn_x)+" "+str(self.pos_pfnn_x))
        # print("PFNN target direction: "+str(true_targetPos2D_x-self.pos_pfnn_x)+" "+str(true_targetPos2D_z-self.pos_pfnn_z))
        ...
        while self.Time_inMySimulation > self.Time_at60fps:
            ...
            valid, alive =self.is_next_pos_valid(episode, self.pos_exact)
            # print("Valid "+str(valid)+" alive "+str(alive))
            if self.settings.realTimeEnvOnline:
                # print("Episode env interaction not hitting objects "+str(pedestrianLastFrameAction.not_hitting_object))
                if  pedestrianLastFrameAction.not_hitting_object !=None:
                    pedestrianLastFrameAction.not_hitting_object = pedestrianLastFrameAction.not_hitting_object == True and valid == True
                else:
                    pedestrianLastFrameAction.not_hitting_object =  valid
                # print("Episode env interaction alive " + str(pedestrianLastFrameAction.not_hitting_object_and_alive))
                if pedestrianLastFrameAction.not_hitting_object_and_alive!=None:
                    valid_case = pedestrianLastFrameAction.not_hitting_object_and_alive == True and alive == True
                else:
                    valid_case = valid and alive
                    pedestrianLastFrameAction.not_hitting_object_and_alive=valid_case
                # print("Valid case " + str(valid_case) + " valid " + str(valid) + " alive " + str(alive))
            else:
                valid_case=valid and alive
            # print("Valid case "+str(valid_case)+" valid "+str(valid)+" alive "+str(alive))
            if not valid:
                # print("Not valid step")
                if not self.settings.realTimeEnvOnline:
                    episode.pedestrian_data[self.id].measures[self.frame, PEDESTRIAN_MEASURES_INDX.hit_obstacles] = 1
                pass

            if np.linalg.norm(step)>0 and not valid_case and not switch_dir:
                #print ("Stop!  stepsize :"+str(np.linalg.norm(step))+" valid case "+str(valid_case)+" switch directions: "+str(switch_dir))
            ...
            episode.pedestrian_data[self.id].agent_high_frq_pos[self.itr_counter, 0] = copy.copy(x)
            episode.pedestrian_data[self.id].agent_high_frq_pos[self.itr_counter, 1] = copy.copy(z)
            episode.pedestrian_data[self.id].avg_speed[self.itr_counter] = copy.copy(realSpeed)
            episode.pedestrian_data[self.id].agent_pose_hidden[self.itr_counter] = self.PFNN.getPrevFrame_SecondHiddenLayerOutput()
            # print("PFNN agent carry out step Hidden pose of frame " + str(self.itr_counter) + " " + str(
            #     np.sum(np.abs(episode.agent_pose_hidden[self.itr_counter])))+" : "+str( episode.agent_pose_hidden[self.itr_counter][0:5]))
            episode.pedestrian_data[self.id].agent_pose[self.itr_counter] = self.PFNN.getCurrentPose()
            #self.itr_counter = self.itr_counter + 1
        ...
        episode.pedestrian_data[self.id].agent_pose_frames[self.frame + 1] = copy.copy(self.itr_counter)
        pose_yaw = self.PFNN.getCurrentPose_Stats_avgYaw()[1:10]
        # print("Yaw before correction " + str(pose_yaw))
        pose_yaw[np.abs(pose_yaw) > 10] = 0
        pose_yaw[np.abs(pose_yaw) < 1e-3] = 0
        if self.settings.realTimeEnvOnline:
            # print("Save yaw in Environment interaction -----------------------------------------------------------")
            pedestrianLastFrameAction.change_in_pose = np.mean(pose_yaw)
        else:
            episode.pedestrian_data[self.id].measures[self.frame, PEDESTRIAN_MEASURES_INDX.change_in_pose] = np.mean(pose_yaw)
        # print self.PFNN.getCurrentPose_Stats_avgYaw()[1:10]
        # print("Yaw "+str(pose_yaw))
        # print ("Agent frame "+str(self.frame)+" yaw "+str(np.mean(pose_yaw))+" saved "+str(episode.measures[self.frame,PEDESTRIAN_MEASURES_INDX.change_in_pose])+" step: "+str(step)+" speed: "+str(desiredSpeed))
        #
        ...
        x, z = self.PFNN.getCurrent2DPos()
        #print ("Done with loop " + str(self.frame + 1) + " agent itr_counter: " + str(self.itr_counter) + " pos: " + str(x) + " pos: " + str(z) + " pose: " + str(sum(episode.agent_pose[self.itr_counter])))
        self.pos_pfnn_x = x
        self.pos_pfnn_z = z
